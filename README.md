...
**Идентификаторы** неймспейснуты ключом вуза (`app/domain/universities.py`):
...
| абитуриент | `spbgu:1645144` | коды вузы выдают свои, и они пересекаются |

Без префикса у абитуриентов совпавший код склеил бы двух разных людей в одну
строку, и модель посчитала бы, что они делят одно место. Пользователю префикс не
показывается (`display_code`), а введённый код ищется сразу по всем вузам
(`find_applicant_keys`) — поэтому одна и та же форма поиска работает и когда код
нашёлся в двух вузах, и когда человек ввёл несколько своих кодов через запятую.
