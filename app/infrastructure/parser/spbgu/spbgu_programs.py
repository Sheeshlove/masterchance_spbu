...
from app.domain import universities
...
def stable_program_code(speciality_code: str, program_name: str, education_form: str = "") -> str:
    """
    Постоянный внутренний код программы СПбГУ.

        stable_program_code("45.04.01", "Славянские языки и литературы (…)")
        → 'spbgu:45.04.01:1f4a9c2b'

    Правило общее для всех вузов и живёт в домене — здесь только подстановка
    ключа СПбГУ, чтобы не тащить его в каждый вызов внутри пакета.
    """
    return universities.stable_program_code(SPBGU, speciality_code, program_name, education_form)


def namespaced_department(speciality_code: str) -> str:
    """'01.04.02' → 'spbgu:01.04.02'."""
    return universities.namespaced_department(SPBGU, speciality_code)


def namespaced_institute(speciality_code: str) -> str:
    """'01.04.02' → 'spbgu:01' — укрупнённая группа направлений."""
    return universities.namespaced_institute(SPBGU, speciality_code)
