...
class Applicant:
    ...
    Код единый для всех вузов, поэтому абитуриент — один на все свои заявки.
    Вуз есть у заявки (через код программы), а не у человека.
    """
    id: str  # Уникальный код поступающего
    university: str = "spbgu"  # в чьих списках код встретился первым
...
class ProgramCompetition:
    ...
    my_consent: bool
    # Соперников, уже подавших согласие в ДРУГОЙ вуз. Держать согласие можно
    # только одно, поэтому такие почти наверняка уйдут и освободят места —
    # это не догадка модели, а факт из списков.
    rivals_committed_elsewhere: int = 0
    # Сколько заявок ждут результатов испытаний — по словам самого вуза
    # («Ожидание результатов ВИ», «На рассмотрении»). Пока таких большинство,
    # конкурс не определён: нули в списке означают не «сдал на ноль», а
    # «ещё не проверено», и разыгрывать по ним места нельзя.
...
class AdmissionDiagnostics:
    ...
    Обе величины считаются ВНУТРИ вуза: «пролетел» в СПбГУ и «пролетел» в ВШЭ —
    разные числа про разные конкурсы. Код абитуриента единый на все вузы,
    поэтому без поля `university` прогон одного вуза затирал бы результат
    другого, и на вкладках стояло бы одно и то же значение.
    """
    applicant_id: str
    p_excluded: float
